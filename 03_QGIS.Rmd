...
- Creating maps for publications and reports 
...
3. Browser panel: Used to locate, inspect and add data to your project. You are able to browse you own computer and additional resources
...
5. Layers list: lists the layers that are available in the current project. Hovering over layers will give some basic information. Layers can be toggled on/off here and right clicking on a layer will give several additional options explored later in the tutorial. 
...
<div class="warning" style='padding:0.1em; background-color:#fee6ce; color:#404040'>
<span>
...
2. From the popup window click the three dots (indicated <span style="color:red">1</span> below) and navigate to the files you wish to add. 
3.Click add <span style="color:red">(2)</span> - this will add the shapefiles to QGIS and then you can close the pop-up window <span style="color:red">(3)</span>. 
...
3. From the "Browser panel" navigate to the folder where the files are located and to add double click on them.  
...
# Checking and setting layer CRS 

1. You can check the layer CRS by hovering your marker over the FAK_HDs layer
2. Right-click on the layer and go to the "Layer CRS" option. Here you will see the layer CRS at the top and some options to change the setting. 
3. Select "Set layer CRS"

```{r cc1, echo=FALSE, fig.cap="", out.width = '70%', fig.align = 'center'}
knitr::include_graphics("images/crs_1.png")
```

4. If you are using a specific CRS you can search for it in the "filter" box. You can also browse the different CRS in the "Predefined Coordinate Reference System" menu. If you select one information about that CRS will be displayed at the bottom. 
5. We will be using the WGS 84 - EPSG: 4326 CRS. If your layer is not yet set simply select this CRS and click OK. 


```{r cc2, echo=FALSE, fig.cap="", out.width = '70%', fig.align = 'center'}
knitr::include_graphics("images/crs_2.png")
```
...
4. It is also possible to search and select features in the attribute table. Let's select all the Admin 2 units named in the Admin 1 region "East". At the top of the attribute table there is a tool bar - click on the filter by form icon ![](images/filter.png)<span style="color:red">(1)</span>. 
5. In the box next to Adm1 <span style="color:red">(2)</span> type "East". 
6. The drop-down menu <span style="color:red">(3)</span> should currently be set on "Contains". Note the other options - "equals to" for example gives an exact name match. Selecting "Contains" returns any name with east in it such as "North east", "South East". In our case we only have one name with "east" so either "contains" or "equals to" will work.
...
10. You can also zoom in on the area you selected by clicking on  "Zoom to Features" 
...
1. We will conduct  basic selection using the select features tool ![](images/select_sh.png). First click on the icon
...
1. To change the color of the map either:
...
```{r point_ex, echo=FALSE, fig.cap="", out.width = '60%', fig.align = 'center'}
knitr::include_graphics("images/excel_point.png")
```
...
```{r lab, echo=FALSE, fig.align = 'center', fig.cap="", out.width = '80%'}
...
```{r csv1, echo=FALSE, fig.cap="", out.width = '100%', fig.align = 'center'}
knitr::include_graphics("images/loadcsv.png")
```


2. Click browser (three black dots) <span style="color:red">1</span> and select the file "annual_admin_data" from the /data folder for this session. 
...
```{r csv2, echo=FALSE, fig.cap="", out.width = '100%', fig.align = 'center'}
...
```{r zs, echo=FALSE, fig.cap="", out.width = '95%', fig.align = 'center'}
...
```{r zs2, echo=FALSE, fig.cap="", out.width = '80%', fig.align = 'center'}
...
```{r zs4, echo=FALSE, fig.cap="", out.width = '70%', fig.align = 'center'}
knitr::include_graphics("images/zone_4.png")
```

```{r zs5, echo=FALSE, fig.cap="", out.width = '50%', fig.align = 'center'}
knitr::include_graphics("images/zone_5.png")
```


10. Navigate to the output folder, give the file name as “FAK_HDs_with_pop” and click Save. Then click Ok.
11. This will again create a new layer. You can remove the “Zonal_Statistics” from the layer pane by right-clicking and selecting remove layer 

```{r z, echo=FALSE, fig.cap="", out.width = '50%', fig.align = 'center'}
...
10. The legend uses the names of layers but these are not always informative. You can change these simply by double clicking on the label in the "Legend Items" panel. Try changing the FAK_HDs label to something like "Number of confirmed malaria cases" 
...
13. The map is starting to look better - try adding a North Arrow ![](images/north.png) and a Scale Bar ![](images/scale.png). Each item on the map has customization options that can be accessed from the "Item properties" tab - try a few of them out. 
...
17. The previous step is mainly used to prevent unwanted movement. We can also lock-in the color symbology so that if you make changes to the main map canvas the print layout remains the same. In the "Item properties" of the "Map 1"  check the tickbox for "lock layers" and "lock styles for layers".
...
18. Return to the main QGIS project and change the color scheme again. What has happened in the print composer now?  

19. There are many different items that can be added to your map such as arrows and tables - find out more [here](https://docs.qgis.org/3.22/en/docs/user_manual/print_composer/composer_items/index.html)
...
2. Choose an appropriate name for the file and save in the output folder.
...
2. Points in polygon  
3. How to access open source spatial data 
4. Plugins - Adding basemaps example 
...
1. Open the FAK_HDs_with_pop attribute table by right-clicking and selecting ![](images/att_table.png)
...
4. While we have editing turned on you can also try to click individual cells of the attribute table. You will be given the option to change the cell contents and is a way to fix any small errors in your dataset. 
5. To turn off editing click the toggle symbol again  ![](images/tog_edit.png). You will be prompted to save or discard your changes - if you are happy click save. Now with the editing turned off try and select the cell contents - you will see it is no longer possible. 
...
1. From the menu bar select "Plugins" and then "Manage and Install plugins"

```{r plug, echo=FALSE, fig.cap="", out.width = '90%', fig.align = 'center'}
knitr::include_graphics("images/plug_1.png")
```

2. The popup window should list the available plugins. In the search bar we will look for "quickmapservices" - this is a popular plugin to access basemaps from Open Street Map, Google etc

```{r plug2, echo=FALSE, fig.cap="", out.width = '90%', fig.align = 'center'}
knitr::include_graphics("images/plug_2.png")
```

3. When you have found the plugin click install. Some plugins will appear in the menu bar but many will appear in the toolbar. For quick map services the tool icon is ![](images/qms.png). CLick the icon and click OSM (open street map). The OSM basemap will appear on your map and in  the layers list. Now you know where Fakeland is!

```{r plug3, echo=FALSE, fig.cap="", out.width = '90%', fig.align = 'center'}
knitr::include_graphics("images/plug_3.png")
```

4. To find other popular base maps you can search the quick map services plugin, again from the ![](images/qms.png) select search QMS. A search panel will appear and looks for other basemaps such a Google or basemaps with satellite imagery 

```{r plug4, echo=FALSE, fig.cap="", out.width = '90%', fig.align = 'center'}
knitr::include_graphics("images/plug_4.png")
```
